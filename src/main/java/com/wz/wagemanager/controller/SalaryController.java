...
//        String deptName = group (DEPT_PATTERN,originalFilename);
        String dateStr = group (DATE_PATTERN,originalFilename);
        Assert.assertTrue ("文件名格式错误,无法获取工资日期",StringUtils.isNotBlank (dateStr));
//        SysDept sysDept = deptService.getDeptByDeptName (deptName);
//        Assert.assertNotNull ("部门["+deptName+"]不存在,请联系管理员添加后重试", sysDept);
...
//            List<ActWork> actWorks = new ArrayList<>();
            List<ActSalary> saveList = new ArrayList<>();
            actList(filePath).forEach(actWork -> {
            ...
            SysDeclare declare = SysDeclare.builder ().declareName (declareName (year,month,sysDept.getDeptName ())).salaryList (saveList).user (sessionUser).status (0).build ();
            actSalaryService.mutilSave(saveList,declare);
        }finally {
            dest.delete ();
        }
        return new PageBean ();
