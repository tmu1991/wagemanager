...
public class SysLogAdvice {
    ...
    public Object doAroundMethod (ProceedingJoinPoint joinPoint, OperInfo annotation) throws Throwable {
        ...
        Map<String, Object> methodArgs = getMethodArgs(parameterNames, joinPoint.getArgs());
        executionTemplate (annotation, methodArgs, sysLog);
    ...
    /**
     * 获取给定方法的参数 返回map key参数名称，value参数的值
     */
    private Map<String, Object> getMethodArgs(String[] params, Object[] args) {
        Map<String, Object> argsMap = new HashMap<>(args.length);
        String argsName;
        for (int i = 0; i < args.length; i++) {
            argsName = params[i];
            if (StringUtils.isBlank(argsName)) {
                continue;
            }
            argsMap.put(argsName, args[i]);
        }
        return argsMap;
    }
    ...
    private void executionTemplate (OperInfo annotation, Map<String, Object> methodArgs, SysLog sysLog) throws IllegalAccessException {
        OperationType type = annotation.type ();
