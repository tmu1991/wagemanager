...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  ,ELEM_VIEW = 'layui-table-view', ELEM_HEADER = '.layui-table-header', ELEM_BODY = '.layui-table-body', ELEM_MAIN = '.layui-table-main', ELEM_FIXED = '.layui-table-fixed', ELEM_FIXL = '.layui-table-fixed-l', ELEM_FIXR = '.layui-table-fixed-r',ELEM_FOOT= '.layui-table-foot', ELEM_TOOL = '.layui-table-tool', ELEM_SORT = '.layui-table-sort', ELEM_EDIT = 'layui-table-edit', ELEM_HOVER = 'layui-table-hover'
  ...
  ,TPL_FOOT = function(options) {
		return options = options || {},
		[
		'<table cellspacing="0" cellpadding="0" border="0" class="layui-table">',
  		 	"<tfoot>",
  		 	"{{# layui.each(d.data.foot, function(i1, item1){ }}",
  		 	  	'<tr id="totalData" class="tr_sum">',
  		 	  	 	"{{# layui.each(item1, function(i2, item2){ }}",
  		 	  	 		'{{# if(item2.fixed && item2.fixed !== "right"){ left = true; } }}',
                ...
                function(){
                  ...
                  return '';
  							...
  							'<td {{#if(item2.isToal){}}id="totalData" class="tr_sum"{{#}}}>',
  								'<div class="layui-table-cell  laytable-cell-{{d.index}}-{{item2.field||i2}}" {{#if(item2.align){}}align="{{item2.align}}"{{#}}}>',
  								'<span>{{item2.title||""}}</span>',
  								"</div>",
  						 	"</td>",
  ...
  ,TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
    ...
    ,"{{# if(d.data.footer){ }}"
      ,'{{# if(left){ }}'
        ,'<div class="layui-table-fixed layui-table-fixed-b">'
          ,'<div class="layui-table-foot">'
            ,TPL_FOOT({fixed:true})
          ,"</div>"
        ,"</div>"
      ,"{{# }; }}"
      ,"{{# if(right){ }}"
        ,'<div class="layui-table-fixed layui-table-fixed-r layui-table-fixed-b">'
          ,'<div class="layui-table-foot">'
            ,TPL_FOOT({fixed:"right"})
          ,"</div>"
        ,"</div>"
      ,"{{# }; }}"
      ,'<div class="layui-table-foot" style="overflow: hidden;">'
        ,TPL_FOOT()
      ,"</div>"
    ,"{{# } }}"
  ...
  Class.prototype.render = function(sets){
    ...
    that.layFooter = reElem.find(ELEM_FOOT)
  ...
  Class.prototype.scrollPatch = function(){
    ...
    if(scollWidth && scollHeight){
      if(!that.elem.find('.layui-table-patch')[0]){
        ...
        that.layFooter.last().find("tfoot tr").append(patchElem.clone());
        that.layHeader.eq(0).find('thead tr').append(patchElem)
      }
    } else {
      that.layFooter.eq(2).find(".layui-table-patch").remove();
  ...
  Class.prototype.events = function(){
    ...
    that.layMain.on('scroll', function(){
      ...
      that.layFooter.scrollLeft(scrollLeft),
